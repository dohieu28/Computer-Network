...
from module1_core.core_infrastructure import Interface, TopologyManager, VirtualRouter
...
def protocol_handler(router: VirtualRouter, incoming_interface, raw_bytes: bytes):
    ...
    print(
        f"  Incoming interface: {getattr(incoming_interface, 'interface_id', incoming_interface)}")
...
def main() -> None:
    ...
    iface0 = Interface(interface_id="eth0",
                       ip_address="10.0.0.1", mac_address="00:11:22:33:44:55")
    iface1 = Interface(
        interface_id="eth1", ip_address="192.168.1.1", mac_address="00:11:22:33:44:66")

    router.add_interface(iface0)
    router.add_interface(iface1)
    router.set_route("0.0.0.0/0", next_hop="10.0.0.2",
                     outgoing_interface="eth0", metric=1)
    router.set_route("192.168.1.0/24", next_hop=None,
                     outgoing_interface="eth1", metric=0, protocol="connected")

    response = router.receive_packet(iface0, b"\x45\x00\x00\x54demo-packet")
    print("Protocol handler response:", response)
    duplicate_response = router.receive_packet(
        iface0, b"\x45\x00\x00\x54demo-packet")
    ...
    topo.add_link("R1", "R2", source_interface="eth0",
                  target_interface="eth0", cost=1, bandwidth="1Gbps")
